...
const buildFull = async () => {
  // rollup 打包的配置信息
  const config = {
    input: path.resolve(yydsRoot, "index.ts"), // 打包入口
...
async function buildEntry() {
  // 读取yyds-ui目录下的所有内容，包括目录和文件
  const entryFiles = await fs.readdir(yydsRoot, { withFileTypes: true });
  // 过滤掉 不是文件的内容和package.json文件  index.ts 作为打包入口
  const entryPoints = entryFiles
    .filter((f) => f.isFile())
    .filter((f) => !["package.json"].includes(f.name))
    .map((f) => path.resolve(yydsRoot, f.name));
  const config = {
    input: entryPoints,
    plugins: [nodeResolve(), vue(), typescript()],
    external: (id: string) => /^vue/.test(id) || /^@yyds-ui/.test(id),
  };
